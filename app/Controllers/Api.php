...
class Api extends ResourceController {
    public function initController(RequestInterface $request, ResponseInterface $response, LoggerInterface $logger) {
        ...
        $this->nearest_location_measure = $this->db->table('tbl_settings')->where('type', 'nearest_location_measure')->get()->getRow()->message ?? '';
    ...
    public function get_news_by_tag() {
        ...
        if ($this->access_key != $this->request->getVar('access_key')) {
            ...
            if ($this->request->getVar('tag_id') && $this->request->getVar('user_id') != '' && $this->request->getVar('language_id')) {
                ...
                $join .= ' LEFT JOIN tbl_location ON tbl_location.id = tbl_news.location_id';
                ...
                if(isset($latitude) && isset($longitude) && $latitude != null && $longitude != null){
                    $order_by = "HAVING distance < $nearest_location_measure OR tbl_news.location_id  = 0 ORDER BY CASE WHEN distance IS NULL THEN 1 ELSE 0 END, distance ASC";
                }else{
                    $order_by = "ORDER BY tbl_news.id DESC";
                }
                $query = $this->db->query('SELECT tbl_news.*, IFNULL(tbl_category.category_name, "") as category_name, IFNULL(tbl_subcategory.subcategory_name, "") as subcategory_name, tbl_location.latitude, tbl_location.longitude, SQRT(POW(111.2 * (tbl_location.latitude - '.$latitude.'), 2) + POW(111.2 * ('.$longitude.' - tbl_location.longitude) * COS(tbl_location.latitude / 57.3), 2)) AS distance FROM tbl_news' . $join . ' ' . $where . ' '.$order_by.' LIMIT ' . $offset . ',' . $limit . '');
                            ...
                            if ($comment_id) {
                                ...
                                if (!empty($res1)) {
                                    ...
                                    $fcmMsg = array(
                                        ...
                                        'message' => 'Like in your comment ' . $res_comment1[0]->message . ' by ' . $get_name[0]->name,
                                        'body' => 'Like in your comment ' . $res_comment1[0]->message . ' by ' . $get_name[0]->name,
                                        'sound' => 'default'
                                    );
    ...
    public function get_news_by_user_category() {
        ...
        if ($this->access_key != $this->request->getVar('access_key')) {
            ...
            if ($this->request->getVar('user_id') && $this->request->getVar('category_id') && $this->request->getVar('language_id')) {
                ...
                $nearest_location_measure = $this->nearest_location_measure;
                if(isset($latitude) && isset($longitude) && $latitude != null && $longitude != null){
                    $order_by = "HAVING distance < $nearest_location_measure OR n.location_id  = 0 ORDER BY CASE WHEN distance IS NULL THEN 1 ELSE 0 END, distance ASC";
                }else{
                    $order_by = "ORDER BY n.id DESC";
                }
                $data = $this->db->query("SELECT n.*,c.category_name, t.latitude, t.longitude, SQRT(POW(111.2 * (t.latitude - '.$latitude.'), 2) + POW(111.2 * ('.$longitude.' - t.longitude) * COS(t.latitude / 57.3), 2)) AS distance FROM tbl_news n JOIN tbl_category c ON n.category_id = c.id JOIN tbl_location t ON n.location_id = t.id WHERE n.language_id = $language_id AND n.status = 1 AND n.description !='' AND n.category_id IN ($category_id) AND (n.show_till >=  '$this->toDate' OR CAST(n.show_till AS CHAR(20)) = '0000-00-00') $order_by LIMIT $offset, $limit")->getResult();
                if ($data) {
                    $data1 = $this->db->query("SELECT n.*,c.category_name, t.latitude, t.longitude, SQRT(POW(111.2 * (t.latitude - '.$latitude.'), 2) + POW(111.2 * ('.$longitude.' - t.longitude) * COS(t.latitude / 57.3), 2)) AS distance FROM tbl_news n JOIN tbl_category c ON n.category_id = c.id JOIN tbl_location t ON n.location_id = t.id WHERE n.language_id = $language_id AND n.status = 1 AND n.description !='' AND n.category_id IN ($category_id) AND (n.show_till >=  '$this->toDate' OR CAST(n.show_till AS CHAR(20)) = '0000-00-00') $order_by")->getResult();
                    $total = count($data1);
                    for ($i = 0; $i < count($data); $i++) {
                        $data[$i]->image = ($data[$i]->image) ? base_url() . '/public/images/news/' . $data[$i]->image : '';
                        if ($data[$i]->content_type == "video_upload") {
                    ...
                    if (!empty($res)) {
                        ...
                        if (!empty($res1)) {
                            ...
                            $fcmMsg = array(
                                ...
                                'type' => 'comment',
                                'news_id' => $news_id,
                                'message' => 'Reply in your comment ' . $res[0]->message . ' by ' . $get_name[0]->name,
                                'body' => 'Reply in your comment ' . $res[0]->message . ' by ' . $get_name[0]->name,
                                'sound' => 'default'
    ...
    public function get_news_by_category() {
        ...
        if ($this->access_key != $this->request->getVar('access_key')) {
            ...
            if (($this->request->getVar('category_id') || $this->request->getVar('subcategory_id')) && $this->request->getVar('user_id') != '' && $this->request->getVar('language_id')) {
                ...
                $limit = ($this->request->getVar('limit')) ? $this->request->getVar('limit') : 10;
                $res = $this->db->table('tbl_news');
                $res->select('tbl_news.*,tbl_category.category_name, tbl_location.latitude, tbl_location.longitude, SQRT(POW(111.2 * (tbl_location.latitude - '.$latitude.'), 2) + POW(111.2 * ('.$longitude.' - tbl_location.longitude) * COS(tbl_location.latitude / 57.3), 2)) AS distance');
                $res->where('tbl_news.language_id', $this->request->getVar('language_id'));
                $res->where('(tbl_news.show_till >= "' . $today . '" OR CAST(tbl_news.show_till AS CHAR(20)) = 0000-00-00)');
                ...
                $res->join('tbl_location', 'tbl_news.location_id = tbl_location.id', 'left');
                $res->where('tbl_news.status', '1');
                $res->where('tbl_news.description != ""');

                if(isset($latitude) && isset($longitude) && $latitude != null && $longitude != null){
                   $res->orderBy('CASE WHEN distance IS NULL THEN 1 ELSE 0 END, distance ASC', '', false)
    ->having('distance < ' . $this->nearest_location_measure)
    ->orHaving('tbl_news.location_id = 0')
    ->limit($limit, $offset);
                }else{
                    $res->limit($limit, $offset)->orderBy('id', 'DESC');
                }
                ...
                if ($data) {
                    ...
                    $res1->select('tbl_news.*,tbl_category.category_name, tbl_location.latitude, tbl_location.longitude, SQRT(POW(111.2 * (tbl_location.latitude - '.$latitude.'), 2) + POW(111.2 * ('.$longitude.' - tbl_location.longitude) * COS(tbl_location.latitude / 57.3), 2)) AS distance');
                    ...
                    $res1->join('tbl_location', 'tbl_news.location_id = tbl_location.id', 'left');
                    $res1->where('tbl_news.status', '1');
                    $res1->where('tbl_news.description != ""');
                    if(isset($latitude) && isset($longitude) && $latitude != null && $longitude != null){
                      $res1->orderBy('CASE WHEN distance IS NULL THEN 1 ELSE 0 END, distance ASC', '', false)
    ->having('distance < ' . $this->nearest_location_measure)
    ->orHaving('tbl_news.location_id = 0')
    ->limit($limit, $offset);
                    }else{
                        $res1->limit($limit, $offset)->orderBy('id', 'DESC');
                    }
                    if ($this->request->getVar('category_id')) {
    ...
    public function get_news() {
        ...
        if ($this->access_key != $this->request->getVar('access_key')) {
            ...
            if ($this->request->getVar('user_id') != '') {
                ...
                $query = $this->db->query('SELECT tbl_news.*, IFNULL(tbl_category.category_name, "") as category_name, IFNULL(tbl_subcategory.subcategory_name, "") as subcategory_name, IFNULL(tbl_location.location_name, "") as location_name, tbl_location.latitude, tbl_location.longitude, SQRT(POW(111.2 * (tbl_location.latitude - '.$latitude.'), 2) + POW(111.2 * ('.$longitude.' - tbl_location.longitude) * COS(tbl_location.latitude / 57.3), 2)) AS distance FROM tbl_news' . $join . ' ' . $where . ' '.$order_by.' LIMIT ' . $offset . ',' . $limit . '');
    ...
    public function register_token() {
        ...
        if ($this->access_key != $this->request->getVar('access_key')) {
            ...
            if ($this->request->getVar('token') && $this->request->getVar('language_id')) {
                $token = $this->request->getVar('token');
                $latitude = $this->request->getVar('latitude') ?? 0;
                $longitude = $this->request->getVar('longitude') ?? 0;
                $language_id = $this->request->getVar('language_id');
                $data = $this->db->table('tbl_token')->where('token', $token)->get()->getRow();
               
                if (empty($data)) {
                    $data = ['token' => $token, 'language_id' => $language_id, 'latitude' => $latitude, 'longitude' => $longitude];
                    ...
                    $edit_id = $data->id;
                    $data2 = ['token' => $token, 'language_id' => $language_id, 'latitude' => $latitude, 'longitude' => $longitude];
                    $this->db->table('tbl_token')->where('id', $edit_id)->update($data2);
                   
                    $response['error'] = "false";
                    $response['message'] = "Device already registered && Location Updated";
                }
            } else {
                $response['error'] = "true";
                $response['message'] = "Please fill all the data and submit!";
    ...
    public function verify_token() {
        ...
        if (!empty($token)) {
            try {
                ...
                if (!isset($payload->iss) || $payload->iss != 'localNow') {
                    $response['error'] = "true";
        ...
        if ($this->access_key != $this->request->getVar('access_key')) {
           ...
           if ($this->request->getVar('action_type') && $this->request->getVar('action_type')=='2') {
            if ($this->request->getVar('news_id')) {
                ...
                if ($this->request->getVar('location_id')) {
    ...
    public function get_videos() {    
        ...
        if ($this->access_key != $this->request->getVar('access_key')) {
            ...
            if ($this->request->getVar('language_id') && $this->request->getVar('language_id') != '') {
                ...
                $res->where('tbl_news.language_id', $this->request->getVar('language_id'));
                $res->where('(tbl_news.show_till >= "' . $today . '" OR CAST(tbl_news.show_till AS CHAR(20)) = 0000-00-00)');
                 $res->whereIn('content_type', explode(",", 'video_upload,video_youtube,video_other'));
                $res->join('tbl_category', 'tbl_news.category_id = tbl_category.id');
                $res->join('tbl_subcategory', 'tbl_news.subcategory_id = tbl_subcategory.id', 'left');
                $res->join('tbl_location', 'tbl_news.location_id = tbl_location.id', 'left');
                $res->where('tbl_news.status', '1');
    ...
    ->limit($limit, $offset);
                ...
                $res1->where('tbl_news.language_id', $this->request->getVar('language_id'));
                $res1->where('(tbl_news.show_till >= "' . $today . '" OR CAST(tbl_news.show_till AS CHAR(20)) = 0000-00-00)');
                 $res1->whereIn('content_type', explode(",", 'video_upload,video_youtube,video_other'));
                $res1->join('tbl_category', 'tbl_news.category_id = tbl_category.id');
                $res1->join('tbl_subcategory', 'tbl_news.subcategory_id = tbl_subcategory.id', 'left');
                $res1->join('tbl_location', 'tbl_news.location_id = tbl_location.id', 'left');
                $res1->where('tbl_news.status', '1');
    ...
    public function get_featured_sections()
    {
        ...
        if ($this->access_key != $this->request->getVar('access_key')) {
            ...
            if ($this->request->getVar('language_id') && $this->request->getVar('language_id') != '') {
                ...
                $latitude = $this->request->getVar('latitude') ?? 0;
                $longitude = $this->request->getVar('longitude') ?? 0;
                $settings_job_link = $this->db->table('tbl_settings')->where('type', 'job_add_link')->get()->getResult();
                $settings_job_add_category = $this->db->table('tbl_settings')->where('type', 'job_add_category')->get()->getResult();
                ...
                if ($data) {
                    ...
                    for ($i = 0; $i < count($data); $i++) {
                        ...
                        $nearest_location_measure = $this->nearest_location_measure;
                        if(isset($latitude) && isset($longitude) && $latitude != null && $longitude != null){
                            $having = "HAVING distance < $nearest_location_measure OR tbl_news.location_id  = 0";
                        }else{
                            $having = "";
                        } 
                        if(isset ($select) && $select != null){
                            ...
                            // print_r('SELECT ' . $select . ' FROM ' . $table . ' ' . $join . '  ' . $where . ' ORDER BY ' . $orderby . ' LIMIT ' . $offset . ',' . $limit . '');
                                $result = $this->db->query('SELECT ' . $select . ', tbl_location.latitude, tbl_location.longitude, SQRT(POW(111.2 * (tbl_location.latitude - '.$latitude.'), 2) + POW(111.2 * ('.$longitude.' - tbl_location.longitude) * COS(tbl_location.latitude / 57.3), 2)) AS distance FROM ' . $table . ' ' . $join . '  ' . $where . ' '.$having.' ORDER BY ' . $orderby . ' LIMIT ' . $offset . ',' . $limit . '')->getResult();
                            }else{
                        ...
                        if ($result) {
                            for ($j = 0; $j < count($result); $j++) {
                                ...
                                // Adding Jobs
                                $allowedIds = explode(',',$settings_job_add_category[0]->message);
                                if(in_array($result[$j]->category_id, $allowedIds)){
                                   $result[$j]->description = $result[$j]->description . " <br/> <br/> <br/> <br/> " . $settings_job_link[0]->message;
                                }
        ...
        if ($this->access_key != $this->request->getVar('access_key')) {
            ...
            for ($i = 0; $i < count($data); $i++) {
                ...
                if(isset ($select) && $select != null){
                     if($table == 'tbl_news'){
                    // print_r('SELECT ' . $select . ' FROM ' . $table . ' ' . $join . '  ' . $where . ' ORDER BY ' . $orderby . ' LIMIT ' . $offset . ',' . $limit . '');
                        $result = $this->db->query('SELECT ' . $select . ', tbl_location.latitude, tbl_location.longitude, SQRT(POW(111.2 * (tbl_location.latitude - '.$latitude.'), 2) + POW(111.2 * ('.$longitude.' - tbl_location.longitude) * COS(tbl_location.latitude / 57.3), 2)) AS distance FROM ' . $table . ' ' . $join . '  ' . $where . ' '.$having.' ORDER BY ' . $orderby)->getResult();
                    }else{
    ...
    public function get_location() {     
        ...
        if ($this->access_key != $this->request->getVar('access_key')) {
            ...
            $offset = ($this->request->getVar('offset')) ? $this->request->getVar('offset') : 0;
            $limit = ($this->request->getVar('limit')) ? $this->request->getVar('limit') : 10;
            $data = $this->db->table('tbl_location')->limit($limit, $offset)->orderBy('id', 'DESC')->get()->getResult();
            if ($data) {
                $data1 = $this->db->table('tbl_location')->get()->getResult();
                $total = count($data1);
                $response['error'] = "false";
                $response['total'] = "$total";
                $response['data'] = $data;
