...
class Table extends ResourceController
{
    ...
    public function news()
    {
        ...
        if ($this->request->getVar('location')) {
            ...
            $where .= " AND n.location_id=" . $location;
        }
    ...
    public function get_all_language()
    {
        ...
        if ($this->request->getVar('search')) {
            $search = $this->request->getVar('search');
            $where = "where (id like '%" . $search . "%' OR UPPER(language) like UPPER('%" . $search . "%'))";
        }
    ...
    public function location()
    {
        ...
        if ($this->request->getVar('search')) {
            $search = $this->request->getVar('search');
            $where = "where (id like '%" . $search . "%' OR location_name like '%" . $search . "%' OR latitude '%" . $search . "%' OR longitude like '%" . $search . "%')";
        }
      
        $query = $this->db->query("SELECT COUNT(*) as total FROM tbl_location $where ");
        $res = $query->getResult();
        foreach ($res as $row1) {
            $total = $row1->total;
        }
        $query1 = $this->db->query("SELECT *  FROM tbl_location $where ORDER BY $sort $order LIMIT $offset , $limit");
        $res1 = $query1->getResult();
        $bulkData = array();
        $bulkData['total'] = $total;
        $rows = array();
